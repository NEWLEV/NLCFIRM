...
router.get('/users', authenticateToken, requireRole('super_admin'), (req, res) => {
  const db = getDb();
...
router.patch('/users/:id', authenticateToken, requireRole('super_admin'), (req, res) => {
  const db = getDb();
  ...
  const result = db.prepare(`UPDATE admin_users SET ${updates.join(', ')} WHERE id = ?`).run(...params);
  ...
  db.prepare(
    'INSERT INTO audit_log (user_id, user_email, action, entity_type, entity_id, details) VALUES (?, ?, ?, ?, ?, ?)'
  ).run(req.user.id, req.user.email, 'update_user', 'admin_users', userId, JSON.stringify(req.body));
